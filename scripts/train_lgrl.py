"""
LGRL PPO training on MiniGrid with stage-based forward-only hierarchy.
...
Artifact naming:
  ...
  - Other single envs are suffixed:  lgrl_unlockpickup.pt, etc.
  - Mix runs are prefixed with "mix":
      lgrl_mix_unlockpickup_gotoobject6x6n2_1to3.pt
...
class HierarchyState:
    ...
    def advance(self, env_idx: int, obs: dict):
        ...
        n = self.n_subgoals_per_env[env_idx]
        if next_stage >= n:
            # All stages done — keep the last active subgoal visible to
            # the text stream; no more subgoal rewards.
...
def save_plots(history, plot_dir, planner_tag, artifact_stem, env_name):
    ...
    fig.suptitle(
        f"LGRL PPO ({planner_tag}){title_suffix} -- {env_name}",
...
def _save_checkpoint(path, model, algo, vocab, update, total_frames,
                     planner_tag, env_name, mix_spec=None):
    torch.save({
...
def main():
    ...
    # ---- Banner -------------------------------------------------------
    print("=" * 70)
    print("  LGRL PPO Training (stage-based)")
    print("=" * 70)
    ...
    if args.resume and os.path.exists(checkpoint_path):
        ...
        saved_planner = ckpt.get("planner")
        if saved_planner is not None and saved_planner != planner_tag:
            raise SystemExit(
                f"Checkpoint planner mismatch: expected {planner_tag!r}, got {saved_planner!r}."
            )
    ...
    while total_frames < TOTAL_FRAMES:
        ...
        if update % PLOT_EVERY == 0:
            save_plots(history, PLOT_DIR, planner_tag, artifact_stem, run_label)
