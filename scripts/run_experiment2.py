"""
run_experiment2.py — Reward balance × mission time penalty ablation.

Tests 6 combinations of (R_MISSION, R_SUBGOAL, MISSION_TIME_COEF) on
DoorKey-5x5 with the rule-based planner.

Conditions:
  R_MISSION=0.5  R_SUBGOAL=0.5  MISSION_TIME_COEF=0.5
  R_MISSION=0.5  R_SUBGOAL=0.5  MISSION_TIME_COEF=0.9
  R_MISSION=0.7  R_SUBGOAL=0.3  MISSION_TIME_COEF=0.5
  R_MISSION=0.7  R_SUBGOAL=0.3  MISSION_TIME_COEF=0.9
  R_MISSION=0.9  R_SUBGOAL=0.1  MISSION_TIME_COEF=0.5
  R_MISSION=0.9  R_SUBGOAL=0.1  MISSION_TIME_COEF=0.9

Output structure:
  logs/experiment2/<condition>/metrics.csv
  logs/experiment2/<condition>/training_curves.png
  checkpoints/experiment2/<condition>.pt

Usage:
    python scripts/run_experiment2.py
    python scripts/run_experiment2.py --total-frames 2000000   # shorter run
"""
...
# Budget parameters (fixed for experiment 2)
MAX_ENV_STEPS       = 250
MAX_SUBGOAL_STEPS   = 250
...
def build_conditions():
    conditions = []
    for r_m, r_s in [(0.5, 0.5), (0.7, 0.3), (0.9, 0.1)]:
        for tc in [0.5, 0.9]:
            conditions.append({
                "name": f"rm{r_m}_rs{r_s}_tc{tc}",
                "r_mission": r_m,
                "r_subgoal": r_s,
                "mission_time_coef": tc,
            })
    return conditions
...
class HierarchyState:
    def __init__(self, num_envs, planner, envs):
...
# ---------------------------------------------------------------------------
# Reward shaping (parametrized)
# ---------------------------------------------------------------------------

def make_reshape_reward(hierarchy_state, r_mission, r_subgoal, mission_time_coef):
...
def run_condition(cond: dict, total_frames: int, log_root: str, ckpt_root: str):
    name = cond["name"]
    r_mission = cond["r_mission"]
    r_subgoal = cond["r_subgoal"]
    mission_time_coef = cond["mission_time_coef"]
    ...
    print(f"  R_MISSION={r_mission}  R_SUBGOAL={r_subgoal}  "
          f"MISSION_TIME_COEF={mission_time_coef}")
    print(f"  Max return = {r_mission + r_subgoal:.1f}  |  Total frames: {total_frames:,}")
    print("=" * 70)
    ...
    reshape_reward = make_reshape_reward(
        hierarchy_state, r_mission, r_subgoal, mission_time_coef,
    )
    ...
    return {"name": name, "r_mission": r_mission, "r_subgoal": r_subgoal,
            "mission_time_coef": mission_time_coef,
...
def main():
    parser = argparse.ArgumentParser(description="Experiment 2: Reward Balance × Time Penalty")
    parser.add_argument("--total-frames", type=int, default=10_000_000)
    args = parser.parse_args()

    log_root = os.path.join(PROJECT_ROOT, "logs", "experiment2")
    ckpt_root = os.path.join(PROJECT_ROOT, "checkpoints", "experiment2")

    conditions = build_conditions()

    print("=" * 70)
    print("  EXPERIMENT 2: Reward Balance × Mission Time Penalty")
    ...
    print("=" * 70)
    for c in conditions:
        print(f"    {c['name']:25s}  R_m={c['r_mission']}  R_s={c['r_subgoal']}  "
              f"TC={c['mission_time_coef']}")
    print("=" * 70)
    ...
    print("=" * 70)
    print(f"  {'Condition':25s} | {'R_m':>4} | {'R_s':>4} | {'TC':>4} | "
          f"{'Avg Return':>11} | {'Avg Steps':>10} | {'Time (s)':>9}")
    print("-" * 80)
    for r in results:
        print(f"  {r['name']:25s} | {r['r_mission']:>4.1f} | {r['r_subgoal']:>4.1f} | "
              f"{r['mission_time_coef']:>4.1f} | {r['final_return']:>11.4f} | "
    ...
    with open(summary_path, "w", newline="") as f:
        w = csv.DictWriter(f, fieldnames=[
            "name", "r_mission", "r_subgoal", "mission_time_coef",
            "final_return", "final_steps", "elapsed",
        ])
        w.writeheader()
        for r in results:
            w.writerow(r)
